...
from app.forms import AnimeForm, MangaForm, MovieForm, TVForm, SeasonForm
...
def import_csv(file, user):
    ...
    for row in reader:
        ...
        match media_type:
            case "tv":
                if TV.objects.filter(media_id=row["media_id"], user=user).exists():
                    logger.warning(f"TV: {row['title']} already exists, skipping...")
                else:
            ...
            case "season":
                if Season.objects.filter(
                    ...
                    logger.warning(
                        f"Season: {row['title']} S{row['season_number']} already exists, skipping..."
                    )
                else:
            ...
            case "episode":
                if Episode.objects.filter(
                    related_season__media_id=row["media_id"],
                    episode_number=row["episode_number"],
                    related_season__user=user,
                ).exists():
                    logger.warning(
                        f"Episode: {row['title']} S{row['season_number']}E{row['episode_number']} already exists, skipping..."
                    )
                else:
                    episodes.append(
                        {
            ...
            case "movie":
                if Movie.objects.filter(media_id=row["media_id"], user=user).exists():
                    logger.warning(f"Movie: {row['title']} already exists, skipping...")
                else:
            ...
            case "anime":
                if Anime.objects.filter(media_id=row["media_id"], user=user).exists():
                    logger.warning(f"Anime: {row['title']} already exists, skipping...")
                else:
            ...
            case "manga":
                if Manga.objects.filter(media_id=row["media_id"], user=user).exists():
                    logger.warning(f"Manga: {row['title']} already exists, skipping...")
                else:
    ...
    Episode.objects.bulk_create(episode_instances)
