...
!!! note
        The import process matches your data with the IDs from the sources that Yamtrack uses. Each media type uses different sources, so make sure to have them properly configured. See [Media Sources](https://github.com/FuzzyGrim/Yamtrack/wiki/Environment-Variables#media-sources) for more information.
