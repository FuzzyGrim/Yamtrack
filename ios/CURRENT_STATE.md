...
- Diary: dark native diary surface using live entries, poster-forward cards, consumed-date fallback formatting, media type chips, ratings, compact tag chips, spoiler/rewatch chips, and no in-screen create button or visibility text.
...
- Diary uses `GET /api/v1/diary/` only; entry creation remains routed through media detail logging.
...
- Diary entry detail/edit navigation is not wired yet; Diary rows are read-only in this pass.
