...
- Home: social-first native landing screen with a compact header, richer in-progress media rail, recent diary entries rendered as activity cards, and passive Friend Feed / People to Follow / Community Lists placeholders until those contracts and screens exist.
...
- Media detail: dark production detail page with bleeding poster/accent hero, lowered safe-area-aware poster placement, real provider/Spine rating chips, expandable synopsis, tracked-state summary with progress display, single-path native-keyboard progress updates for in-progress books/games, media-specific metadata, reviews, TV season detail pages with explicit show + season titles and clear episode lists, seasons/episodes, cast/crew/authors, live related sections, poster customization for TMDB movies/TV, and a page-only custom bottom nav overlay.
...
- Tags: Profile -> Tags now shows a searchable wrapping pill grid of personal tags sorted by usage, with usage counts and no `#` prefix. Diary-log tags open a native tag detail page with a top Diary/Grid segmented control, cleaned-up title text, filtered diary logs, and a unique-media poster grid.
...
- Profile Lists: list overview rows are taller media cards with title, item count, and a horizontally scrollable poster preview strip. The overview keeps the main tab bar visible; list detail remains an immersive pushed screen with its full item grid.
...
- Profile Lists uses `GET /api/v1/lists/` for overview rows and optional capped `preview_items` poster strips, plus `GET /api/v1/lists/{id}/` for full list detail items.
- Home uses `GET /api/v1/me/`, `GET /api/v1/tracking/?media_type=<type>&status=In%20progress`, and the first page of `GET /api/v1/diary/`; it does not consume undocumented feed/list/people endpoints.
...
- Profile Tags requires backend support for `all=true` on `GET /api/v1/diary/tags/`; without it, the page can only show whatever capped results the endpoint returns.
...
- Home social feed, people suggestions, and community lists are placeholders only until documented API response shapes and matching iOS screens exist.
