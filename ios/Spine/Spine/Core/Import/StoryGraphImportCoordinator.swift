...
enum StoryGraphImportJobPhase: Equatable {
    case idle
...
extension Notification.Name {
    static let storygraphImportDidSucceed = Notification.Name("storygraphImportDidSucceed")
}

@MainActor
@Observable
final class StoryGraphImportCoordinator {
    var phase: StoryGraphImportJobPhase = .idle
    ...
    private func runImport(fileURL: URL, mode: ImportMode) async {
        guard fileURL.pathExtension.lowercased() == "csv" else {
            phase = .failed(message: "Please upload the .csv export from StoryGraph, not a ZIP file.")
            return
        }
        ...
        do {
            ...
            let response = try await importRepository.queueStoryGraphImport(
                fileData: file.data,
                fileName: file.name,
                mode: mode,
    ...
    private func handle(task: ImportTaskStatus, taskId: String, startedAt: Date) -> Bool {
        ...
        case "SUCCESS":
            clearPersistedJob()
            phase = .succeeded(message: task.result?.trimmedNonEmpty ?? "StoryGraph import complete.")
            NotificationCenter.default.post(
                name: .storygraphImportDidSucceed,
                object: self,
        ...
        case "FAILURE":
            clearPersistedJob()
            phase = .failed(message: task.result?.trimmedNonEmpty ?? "StoryGraph import failed.")
            return true
        default:
            if Date().timeIntervalSince(startedAt) >= timeout {
                phase = .failed(message: Self.timeoutMessage)
    ...
    private static func statusLabel(for status: String) -> String {
        ...
        case "PENDING", "QUEUED":
            "Queued..."
        case "STARTED":
            "Importing books and diary entries..."
        case "SUCCESS":
            "Import complete."
        case "FAILURE":
            "Import failed."
    ...
    private nonisolated static func readFile(_ url: URL) async throws -> (data: Data, name: String, size: Int64?) {
        try await Task.detached(priority: .userInitiated) {
            ...
            let fileName = trimmedFileName.isEmpty ? "storygraph.csv" : trimmedFileName
    ...
    private enum Keys {
        static let taskId = "storygraphImport.taskId"
        static let mode = "storygraphImport.mode"
        static let startedAt = "storygraphImport.startedAt"
    }
}
